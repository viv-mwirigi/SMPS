...
from swpps.physics.van_genuchten import potential_from_water_content, water_content_from_potential
...
def theta_to_psi(
    ...
    *,
    ...
    """
    theta_safe = clip_theta(theta, params, eps=(
        clips.theta_eps if clips else 1e-6))
    ...
    psi_arr = np.array([potential_from_water_content(float(t), params)
                       for t in np.asarray(theta_safe, dtype=float)])
    if clip_output and clips is not None:
...
def psi_to_theta(
    ...
    *,
    ...
    theta_arr = np.array([water_content_from_potential(
        float(p), params) for p in np.asarray(psi_kpa, dtype=float)])
    if clip_output:
...
def theta_thresholds_for_psi(
    ...
    """
    items = psi_thresholds_kpa.items() if hasattr(
        psi_thresholds_kpa, "items") else psi_thresholds_kpa
