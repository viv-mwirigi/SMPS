...
class IrrigationDecisionEngine:
    ...
    def _calculate_irrigation_amount(
        ...
        if self.config.vg_params is not None:
            ...
            theta_target = water_content_from_potential(float(self.refill_target), vg)

            # If already wetter than target, no refill.
            dtheta = max(0.0, float(theta_target - theta_now))
            gross_amount = dtheta * float(self.config.root_depth_m) * 1000.0  # m -> mm
        else:
    ...
    def _create_decision(
        ...
        return IrrigationDecision(
            ...
            recommended_amount_mm=float(amount_mm) if action in (IrrigationAction.IRRIGATE_NOW, IrrigationAction.SCHEDULE) else None,
        )
